...
function relocateSystemEntries(body, keptSystem, movedSystem) {
    if (movedSystem.length === 0 || !Array.isArray(body.messages))
        return;
    ...
    const blocks = movedSystem.map((block, index) => ({
        ...block,
        text: `${block.text ?? ""}${index < movedSystem.length - 1 || typeof firstUser.content === "string"
            ? "\n\n"
            : ""}`,
    }));
    ...
    if (Array.isArray(firstUser.content))
        firstUser.content.unshift(...blocks);
    return blocks.at(-1);
}
function applyPromptCaching(body, movedSystemBoundary) {
...
function markCacheBoundary(block) {
    if (!block || hasCacheControl(block["cache_control"]))
        return;
    block["cache_control"] = { type: "ephemeral" };
}
function findLastCustomTool(tools) {
    for (let index = (tools?.length ?? 0) - 1; index >= 0; index--) {
        if (tools[index].type === undefined)
            return tools[index];
    }
}
function findPreviousMessageBoundary(messages) {
    ...
    return Boolean(hasCacheControl(body.cache_control) ||
        ...
        (Array.isArray(body.messages) &&
            body.messages.some((message) => Array.isArray(message.content) &&
                message.content.some((block) => hasCacheControl(block["cache_control"])))));
}
 ...
 * the drop strategy.
 */
export function resolveToolRepairMode(env = process.env) {
...
function hasThinkingBlock(message) {
...
function toolUseHasAdjacentResult(messages, index, id) {
 ...
 */
function dropPass(messages) {
    let changed = false;
    const droppedToolUseIds = [];
    const droppedToolResultIds = [];
    const omittedThinkingTurns = [];
    const out = [];
    messages.forEach((message, index) => {
        ...
        out.push(filtered.length === message.content.length
            ? message
...
export function synthesizeMissingToolResults(messages) {
    ...
    for (let i = 0; i < pass1.length; i++) {
        ...
        const presentIds = next && Array.isArray(next.content)
            ? new Set(next.content
                .map(toolResultIdOf)
                .filter((id) => id !== undefined))
            : new Set();
...
export function applyToolRepair(messages, mode) {
    return mode === "drop"
        ? repairToolPairs(messages)
        : synthesizeMissingToolResults(messages);
}
export function transformBody(body, mode = resolveToolRepairMode()) {
    ...
    try {
        ...
        const movedSystemBoundary = relocateSystemEntries(parsed, keptSystem, movedSystem);
        ...
        if (!hasInputPromptCaching)
            applyPromptCaching(parsed, movedSystemBoundary);
