...
import { CLAUDE_CODE_OAUTH_METADATA_KEY, CLAUDE_CODE_OAUTH_METADATA_VALUE, } from "./oauth-method.js";
...
export function claudeSubscriptionFetch(accessToken, upstream, source) {
    ...
    const resolveAccount = () => source === undefined ? getActiveAccount() : getAccountBySource(source);
    return async (input, init = {}) => {
        const requestBody = input instanceof Request && init.body === undefined
        ...
        if (typeof requestBody === "string") {
            try {
                modelID =
                    JSON.parse(requestBody).model ?? "unknown";
            }
            catch { }
        }
        ...
        if (!credentials)
            credentials = await getCredentialsWithBackoff({}, account);
        ...
        const sendWithToken = (currentToken, excludedBetas = excluded) => fetchWithRetry(requestURL, {
            ...requestInit,
            body: transformedBody,
            headers: buildRequestHeaders(input, requestInit, currentToken, modelID, excludedBetas),
        }, 3, send);
        let response = await sendWithToken(token);
        for (let attempt = 0; response.status === 401 && attempt < 2; attempt++) {
            let candidate = reloadCredentialsFromSource(account);
        ...
        for (let attempt = 0; attempt < LONG_CONTEXT_BETAS.length; attempt++) {
            ...
            response = await fetchWithRetry(requestURL, {
                ...requestInit,
                body: transformedBody,
                headers: buildRequestHeaders(input, requestInit, token, modelID, getExcludedBetas(modelID)),
            }, 3, send);
        }
        if (!response.ok)
...
export function createClaudeSubscription(options) {
    if (options[CLAUDE_CODE_OAUTH_METADATA_KEY] !== CLAUDE_CODE_OAUTH_METADATA_VALUE)
        return createAnthropic(options);
    const accessToken = typeof options.apiKey === "string" ? options.apiKey : "";
    const upstream = typeof options.fetch === "function" ? options.fetch : undefined;
