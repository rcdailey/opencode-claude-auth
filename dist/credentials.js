...
export function setActiveAccountSource(source) {
 ...
 */
export function getAccountBySource(source) {
...
export function saveAccountSource(source) {
    try {
        const path = getAccountStateFile();
 ...
 */
export async function forceRefreshActiveAccount(refresh = refreshViaOAuth, account = getActiveAccount()) {
...
export async function getCachedCredentials(account = getActiveAccount()) {
    if (!account)
        return null;
    const now = Date.now();
 ...
 */
export async function getCredentialsWithBackoff(opts = {}, account = getActiveAccount()) {
 ...
 */
export function getActiveRefreshFailureKind(account = getActiveAccount()) {
    const source = account?.source;
    if (!source)
        return null;
...
export function reloadCredentialsFromSource(account = getActiveAccount()) {
    if (!account)
        return null;
    let reloaded;
    try {
